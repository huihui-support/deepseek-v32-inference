#!/usr/bin/env python3
"""
CPU-based inference for deepseek-ai/DeepSeek-V3.2 → Full model loaded into RAM at once
"""
...
# Model directory (must contain model.safetensors.index.json and shard files)
MODEL_PATH = "deepseek-ai/DeepSeek-V3.2"
# Block size used for FP8 block-wise scaling
BLOCK_SIZE = 128


@dataclass
class ModelArgs:
    # Hyper-parameters of DeepSeek-V3.2 (7168 dim, 61 layers, 256 experts) – must exactly match the original model
    vocab_size: int = 129280
    dim: int = 7168
    inter_dim: int = 18432
    moe_inter_dim: int = 2048
    n_layers: int = 61
    n_dense_layers: int = 3                # First 3 layers are dense MLPs
    n_heads: int = 128
    n_routed_experts: int = 256            # Total number of experts
    n_shared_experts: int = 1              # V3.2 has only 1 shared expert
    n_activated_experts: int = 8           # Top-8 routing
...
def dequantize_fp8(weight_fp8: torch.Tensor, scale_inv: torch.Tensor) -> torch.Tensor:
    ...
    # Compute number of 128×128 blocks
    ...
    # Reshape into blocks and apply per-block scaling
...
class TensorLoader:
    """Load the entire model into RAM at once (no mmap anymore)."""
    ...
    def __init__(self, model_path: str):
        ...
        # Pre-load only the actual weight tensors (not the scale_inv ones)
        weights_to_preload = [
            name for name in self.index["weight_map"].keys()
            if name.endswith(".weight") and "weight_scale_inv" not in name
        ]

        print(f"Found {len(weights_to_preload)} main weight tensors, loading them all into RAM...")
        for name in tqdm(weights_to_preload, desc="Loading weights → RAM", colour="cyan"):
            _ = self.load(name)                     # Triggers loading + dequantization + caching

        total_tensors = len(self.cache)
        total_gb = sum(t.numel() * t.element_size() for t in self.cache.values()) / 1e9
        print(f"Loading finished! Cached {total_tensors} tensors (including auto-loaded scale_inv)")
        print(f"Memory usage ≈ {total_gb:.1f} GB (roughly 1.4–1.5 TB)")
    ...
    def load(self, name: str, cache: bool = True) -> torch.Tensor:
        ...
        # Proper FP8 dequantization when needed
        if tensor.dtype == torch.float8_e4m3fn:
            scale_name = name.replace(".weight", ".weight_scale_inv")
            if scale_name in self.index["weight_map"]:
                scale_inv = self.load(scale_name, cache=False)
                tensor = dequantize_fp8(tensor, scale_inv)
...
class DeepSeekV32CPU:
    def __init__(self, model_path: str, args: ModelArgs):
        self.args = args
        # This triggers the full in-RAM loading
    ...
    def attention(self, x: torch.Tensor, layer: int, start_pos: int) -> torch.Tensor:
        """DeepSeek-V3 style attention with YaRN + LoRA structure (no KV cache)."""
    ...
    def rms_norm_inline(self, x: torch.Tensor, layer: int, name: str) -> torch.Tensor:
        """RMSNorm used inside attention LoRA blocks (some layers may not have it)."""
    ...
    def mlp_dense(self, x: torch.Tensor, layer: int) -> torch.Tensor:
        """Dense SwiGLU MLP used in the first 3 layers."""
    ...
    def mlp_moe(self, x: torch.Tensor, layer: int) -> torch.Tensor:
        """MoE block – routing + shared expert + on-demand routed experts."""
...
def main():
    print("="*60)
    print("DeepSeek-V3.2 CPU Inference – Full-model-in-RAM version")
    print("Requires ~1.4–1.6 TB of RAM, extremely fast after loading!")
    print("="*60)
    ...
    prompt = "What is 2 + 2?"
    print(f"\nPrompt: {prompt}\n" + "-"*40)
    model.generate(prompt, max_new_tokens=300, temperature=0.7)
    print("-"*40)


if __name__ == "__main__":
    print("Warning: About to load ~1.4 TB of weights into RAM!")
    input("Press Enter to continue (you need a machine with >2 TB RAM)...")
    main()